...
<title>Sale Hunter - User Login</title>
<!-- for-mobile-apps -->
...
// connect to mongodb
   ...
   $db = $m->dealhunter;
   $collection = $db->users;
if(!empty($_POST['fname']) && !empty($_POST['lname']) && !empty($_POST['username']) && !empty($_POST['password']) && !empty($_POST['password_again']) && !empty($_POST['email']) && !empty($_POST['phone']))
 { 
   ...
   $phone = $_POST['phone'];
	...
	if($x)
	{
		if(md5($password)==md5($password_again))  
		{
